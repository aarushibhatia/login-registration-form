...
exports.populateData = (username, name, password) => {
        ...
        connection.query(sql, [username], function (error, result) {
            if (error) {
                ...
                if (result[0].userExists > 0) {
                    ...
                    connection.query("INSERT INTO usersDB (username, fullName, password) VALUES(?, ? , ?)", [username, name, password], function (error, result) {
                        if (error) {
                            LOG.error(error);
                            return resolve({ result: false });
                        }
